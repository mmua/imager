...
def crop(pp):
    print "write data"
    res = yield pp.write('{"src": "test.jpg", "dst": "146x97.jpg", "cmd": "crop", "resize": "146x97"}' + "\n")
    print res

if __name__ == '__main__':
    pp = ImagerProtocol()
    subprocesses = {}
    command = ['/var/www/images.f1news.ru/scripts/resize_crop_image.py']
    subprocess = reactor.spawnProcess(pp, command[0], command, {})
    print subprocess
    subprocesses[subprocess.pid] = subprocess
    reactor.callLater(3, crop, pp)
    reactor.run()
